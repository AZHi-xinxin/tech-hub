...
顶栏两页：**群聊**（主界面）与**进度**（任务看板）。审批不常驻——有待批准事项时群聊顶部自动出现置顶红卡（谁申请/做什么/影响范围/30 分钟截止/批准一次/拒绝），两页标签同时亮红点。进度页分五区：需要你处理、进行中、阻塞·失败、历史（近 48h 折叠）、系统记录（门铃自动任务沉底只记一行）；任务卡只给人看五项：目标/谁在做/当前步骤/更新时间/结果或阻塞原因。红点语义：群聊红点=有比你更新的非本人消息（你发完话即灭），进度红点=自上次打开进度页后有新任务变化（进页即已读）。
...
- [tech-hub Android 壳](tech-hub-apk/README.md)：手机 WebView 壳工程（未读轮询/原生通知/角标），构建步骤与实现要点

## 版本

- **0.1.1**（当前）：消息游标翻页、房间折叠+折叠卡片、门铃哨兵、任务账本、审批流、气泡 UI、微信式自适应输入框、附件通道（发图/发文件）、群聊/进度双页+待办红卡+双红点、Android 壳（见 tech-hub-apk）
